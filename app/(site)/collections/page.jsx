...
import { getCollectionProducts, PLACEHOLDER_IMAGE } from '@/lib/catalog'

export const metadata = {
  title: 'Collections',
  description: 'Curated edits from the Knock Nation Bag range — new arrivals, the featured collection and current reductions.',
  alternates: { canonical: '/collections' },
  openGraph: { title: 'Collections | Knock Nation Bag', url: '/collections' },
}

export default async function CollectionsPage() {
  const collections = await Promise.all(COLLECTIONS.map((c) => getCollectionProducts(c.slug)))
  ...
  return (
    <>
      <PageHeader
        ...
        title="Collections"
        description="Different ways into the range."
        breadcrumbs={[{ label: 'Collections' }]}
      />

      <Container className="py-10 md:py-14 xl:py-16">
        <ul className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 xl:gap-6">
          {collections.map(({ collection, products }) => {
            const cover = products.find((p) => p.image !== PLACEHOLDER_IMAGE)
            return (
              <li key={collection.slug}>
                <Link
                  href={`/collections/${collection.slug}`}
                  className="group relative block h-[220px] overflow-hidden rounded-card bg-surface-muted md:h-[280px] xl:h-[340px] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold"
                >
                  <Image
                    src={cover?.image ?? PLACEHOLDER_IMAGE}
                    alt={cover?.imageAlt ?? ''}
                    fill
