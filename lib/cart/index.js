...
async function cookieCartId() {
  ...
  const { data, error } = await createAdminClient().from('carts').select('id').eq('id', cookieId).maybeSingle()
  // An error is not "no cart": treating it so would silently start a new one.
 ...
 *
 * `strict`: a database error throws instead of reading as "no cart". Cart
 * mutations use it, so a failed lookup can never silently start a new cart.
 * Best-effort callers (badge count, post-payment tidy-up) keep the default.
 */
export async function findCartId(userId, { strict = false } = {}) {
  try {
    const fromCookie = await cookieCartId()
    if (fromCookie) return fromCookie
    const uid = userId === undefined ? (await getShopper()).userId : userId
    return await accountCartId(uid)
  } catch (error) {
...
export async function loadCart() {
  ...
  let shopper, settings, cartId, rows
  try {
    let fromCookie
    ;[shopper, settings, fromCookie] = await Promise.all([getShopper(), getStoreSettings(), cookieCartId()])
    cartId = fromCookie ?? (await accountCartId(shopper.userId))
    ...
    if (result.error) throw new Error(`cart items: ${result.error.message}`)
    rows = result.data
  } catch (error) {
    // Never show a real cart as empty because the database could not be read.
